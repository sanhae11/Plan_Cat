...
public class MonthlyFragment extends Fragment {
    ...
    private MonthlyTodoViewModel todoViewModel;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
        ...
        RecyclerView recyclerView = rootView.findViewById(R.id.monthlyTodoRecyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext())); //parameter 원래는 this였음
        recyclerView.setHasFixedSize(true);

        final MonthlyTodoRecyclerAdapter adapter = new MonthlyTodoRecyclerAdapter();
        recyclerView.setAdapter(adapter);

        todoViewModel = ViewModelProviders.of(this).get(MonthlyTodoViewModel.class); //parameter 원래 this 였는데 오류 안나서 안 바꿈
        ...
        adapter.setOnItemClickListener(new MonthlyTodoRecyclerAdapter.OnItemClickListener() { //할 일 클릭 시 edit to do 화면으로 이동
            @Override
        ...
        adapter.setOnItemLongClickListener(new MonthlyTodoRecyclerAdapter.OnItemLongClickListener() { //할 일 꾹 누르면 delete to do 다이얼로그 창 뜸
            @Override
        ...
        adapter.setOnStatusCheckBoxChanged(new MonthlyTodoRecyclerAdapter.OnStatusCheckBoxChangeListener() { //checkbox 클릭할 때마다 db 업데이트
            @Override
